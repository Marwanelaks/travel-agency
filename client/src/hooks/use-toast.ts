...
const ToastItem: React.FC<{ toast: Toast; onDismiss: (id: string) => void }> = ({
  ...
  return (
    <div
      className={`relative flex items-center justify-between w-full max-w-sm p-4 rounded-md shadow-lg ${variantStyles[variant]}`}
      role="alert"
    >
      ...
      <button
        ...
        className="ml-4 text-foreground/50 hover:text-foreground focus:outline-none"
        aria-label="Close"
      >
